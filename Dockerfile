...
RUN \
  ...
  if [ -z ${TVHEADEND_COMMIT+x} ]; then \
    TVHEADEND_COMMIT=$(curl -sX GET https://api.github.com/repos/mr-onion-2/tvheadend/commits/master \
    | jq -r '. | .sha'); \
  fi && \
  mkdir -p \
    /tmp/tvheadend && \
  git clone https://github.com/mr-onion-2/tvheadend.git /tmp/tvheadend && \
  cd /tmp/tvheadend && \
